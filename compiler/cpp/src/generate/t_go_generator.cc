...
void t_go_generator::generate_service_server(t_service* tservice) {
	...
	f_service_ << indent() << "func (p *" << serviceName
		...
		"(processor func(seqId int32, iprot, oprot thrift.TProtocol)(success bool, err thrift.TException), ok bool) {" << endl;
	...
	for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
		...
		f_service_ << indent() << "  return p.Process_" << publicize((*f_iter)->get_name()) << ", true" << endl;
	}
	f_service_ << indent() << "default:" << endl;
	if (is_extend){
		//defer to parent if not in child.
		f_service_ << indent() << "  return p." << extends_processor << ".GetProcessorFunction(key)" << endl;
	}
	else{
		f_service_ << indent() << "  return nil,false" << endl;
	}
	f_service_ << indent() << "}" << endl;
	...
	f_service_ << indent() << "    return processor(seqId, iprot, oprot)" << endl;
	f_service_ << indent() << "  }" << endl;
